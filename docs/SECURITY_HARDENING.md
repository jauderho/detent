...
| Content written to allow-listed targets is NOT validated: smb.conf `root preexec`, dnsmasq `dhcp-script`, `/etc/fstab`, `/etc/exports` `no_root_squash`, ifupdown `up` lines can still execute as root (STAGE3 H23 step 2, not yet built) | a compromised worker gaining root through config content | _none yet_ | **not yet — H23 step 2** (monitor-side re-validation + per-module exec-directive deny-list) |
...
| External validators (planned — not yet gating `apply`) | applying a config that would fail to load at the service level | `crates/detent-platform/src/privsep/monitor.rs` (`RunCheck`), `service/checks.rs`, `crates/detent-ops/src/engine.rs:402` (`run_checks` only in `plan()`) | **planned** — `RunCheck` exists and `plan()` gates on it, but `apply` writes without calling validators; `run_check_reports_success_through_a_working_check_runner`, `run_check_maps_a_failed_hook_to_an_io_error`, `run_check_rejects_an_unknown_check_id` cover the mechanism only — monitor would be killed in confined `serve` before H6 (H5, H6) |
...
| Commit-confirm: apply arms a timer, unconfirmed apply rolls back automatically; does not yet survive a monitor restart (H1 — `recover_pending` has no production caller; timer is in-memory only) | an apply that breaks network/service reachability locking the admin out | `crates/detent-platform/src/privsep/monitor.rs` (`PENDING_COMMIT_MARKER`, `recover_pending`) | `an_unconfirmed_commit_rolls_back_when_the_timer_fires`, `a_commit_confirm_module_arms_a_window_that_confirm_closes`, `recover_pending` currently tested only, not wired at monitor startup (H1) |
...
| Audit log: intent fail-closed (a `Started` record that cannot be persisted aborts the mutation via `AuditUnavailable`), outcome/denial best-effort; not append-only or tamper-evident (worker-owned file) until M3; journald after M12 is the unrewritable copy, no secrets/full bodies, includes prev/new hash | inability to reconstruct what happened after an incident; accidental secret leakage into the audit trail | `crates/detent-ops/src/audit.rs` (`FileSink` under worker-writable `state_root`), `crates/detent-ops/src/engine.rs:174-179` (intent `Started` → `AuditUnavailable`), `crates/detent-ops/src/engine.rs:210-224` (`emit` returns `Result`, logs at `error` + `tracing::info` to journald) | `a_record_carries_hashes_and_an_error_id_but_no_payload`, `the_file_sink_appends_one_line_per_record_and_reads_them_newest_first` (`audit.rs`); `an_unwritable_audit_sink_refuses_the_mutation` (intent fail-closed at `engine.rs:174-179`), outcome/denial best-effort pinned; not tamper-evident, worker-owned file can be rewritten (H7, M3 — journald after M12 is intended) |
