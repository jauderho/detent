...
| Concurrent-connection cap | remote attacker; connection-exhaustion DoS | `crates/detent-web/src/server.rs` | `a_connection_over_the_cap_is_dropped_without_a_handshake` (`tests/tls.rs`) |
...
| Short-lived certs (ACME `shortlived` profile, dns-01, device-attest-01) | stolen-cert reuse window | `detent-acme` (Phase 6) | **not yet — Phase 6.** `detent-acme/Cargo.toml` has no dependencies yet; the crate is a stub. |
...
| `healthz` unauthenticated, constant body, no version leak | information disclosure via a pre-auth endpoint; version fingerprinting | `crates/detent-web/src/server.rs` | `healthz_answers_a_constant_body_and_nothing_else` (`server.rs`); `healthz_is_reachable_through_the_assembled_router`, `openapi_document_is_unauthenticated` (`api/integration_tests.rs`) |
...
| Control | Defends against | Implemented in | Proven by |
|---|---|---|---|
| Privilege separation: monitor (tiny, blocking, no TLS/HTTP) / worker (uid `detent`, no caps, network-facing) over a socketpair | a compromised network-facing worker escalating to root | `crates/detent-platform/src/privsep/monitor.rs`, `worker.rs` | `hooks_confine_both_roles_across_a_forked_pair` (`sandbox/linux.rs`); `dispatch_requires_the_handshake_before_anything_else`, `a_second_hello_is_a_handshake_error` (`monitor.rs`) |
| Closed request set; no path/unit name crosses the socket (ids index allow-lists computed at startup) | a compromised worker requesting an arbitrary file/unit via the privileged channel | `crates/detent-platform/src/privsep/allowlist.rs`, `proto.rs` | `ids_outside_the_tables_resolve_to_nothing`, `ids_are_dense_indices_in_declaration_order` (`allowlist.rs`) |
...
| External validators run before apply | applying a config that would fail to load at the service level | `crates/detent-platform/src/privsep/monitor.rs` (`RunCheck`), `service/checks.rs` | `run_check_reports_success_through_a_working_check_runner`, `run_check_maps_a_failed_hook_to_an_io_error`, `run_check_rejects_an_unknown_check_id` (`monitor.rs`); `crates/detent-platform/tests/service_checks.rs` |
...
| Commit-confirm: apply arms a timer, unconfirmed apply rolls back automatically; survives a monitor restart | an apply that breaks network/service reachability locking the admin out | `crates/detent-platform/src/privsep/monitor.rs` (`PENDING_COMMIT_MARKER`, `recover_pending`) | `an_unconfirmed_commit_rolls_back_when_the_timer_fires`, `a_commit_confirm_module_arms_a_window_that_confirm_closes`, `rollback_commit_restores_the_file_and_is_audited` (`crates/detent-ops/tests/engine.rs`); `rollback_commit_after_the_deadline_already_fired_answers_unknown`, `recover_pending_reports_none_when_no_marker_exists`, `recover_pending_counts_a_failed_restore` (`monitor.rs`) |
...
| Audit log: append-only, no secrets/full bodies, includes prev/new hash | inability to reconstruct what happened after an incident; accidental secret leakage into the audit trail | `crates/detent-ops/src/audit.rs` | `a_record_carries_hashes_and_an_error_id_but_no_payload`, `the_file_sink_appends_one_line_per_record_and_reads_them_newest_first` (`audit.rs`) |
...
| 7-day cooldown on new dependency versions (ADR-011) | a just-published, not-yet-scrutinized malicious release landing immediately | `docs/adr/ADR-011-supply-chain-cooldown.md`; CI | CI job: `.github/workflows/ci.yml` step "cargo cooldown check (7-day supply-chain cooldown, ADR-011)" (`cargo cooldown check --locked`). |
