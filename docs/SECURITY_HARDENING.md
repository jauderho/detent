...
| Audit log: best-effort (not append-only or tamper-evident), no secrets/full bodies, includes prev/new hash | inability to reconstruct what happened after an incident; accidental secret leakage into the audit trail | `crates/detent-ops/src/audit.rs` (`FileSink` under worker-writable `state_root`), `crates/detent-ops/src/engine.rs:205-207` (`emit` logs-not-propagated) | `a_record_carries_hashes_and_an_error_id_but_no_payload`, `the_file_sink_appends_one_line_per_record_and_reads_them_newest_first` (`audit.rs`); best-effort pinned at `engine.rs:1834` (`audit_fail_open_is_pinned`); not tamper-evident, worker-owned file can be rewritten (H7, M3 — journald after M12 is intended) |
