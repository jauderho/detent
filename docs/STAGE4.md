...
1. **Provider HTTPS transport** for Cloudflare, acme-dns and deSEC. Use the hyper-rustls client that `detent-acme` `order.rs` already builds. **The docs are wrong that a new dependency is needed**; fix `providers.rs:51-60` and `Cargo.toml:12-15`. Add TSIG for RFC 2136 with `hmac`/aws-lc (already in the workspace). Add provider selection in `[acme]`. Tests: recorded fixtures, and a **log-capture** test that no secret is logged.
   ...
   - **Renewal loop (item 2): BLOCKED on an owner decision.** The confined worker cannot open an outbound connection: the worker seccomp table (`crates/detent-platform/src/sandbox/seccomp.rs`, `WORKER`) has no `connect`, `getsockopt`, `getpeername` or `poll`/`ppoll`, and its default is `EPERM`. Every ACME order and provider call would fail with `EPERM`. Options: (a) add those calls to `WORKER` (proved with a live `strace -f`, as the table requires); this gives the network-facing process outbound access; (b) run the ACME client in its own child process with its own confinement, fed the provider secret before the fork. Design notes for either option: the worker runtime is current-thread and `DnsProvider` calls block (use `spawn_blocking` or a separate thread); `present_challenges` needs `publish: &(dyn Fn(&DnsRecord) + Sync)` to be `Send`; ARI needs a DER form of `ari_identifier`; `[acme]` has no EAB field yet.
