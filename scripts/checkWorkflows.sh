...
get_workflow_name_from_file() {
  ...
  # No match (or a SIGPIPE from head) is not an error here - the caller falls
  # back to the filename below - so swallow the pipeline's status.
  local name
  name=$(grep -E "^name:" "$workflow_file" | head -1 | sed 's/name:[[:space:]]*//' | sed 's/^["\x27]//' | sed 's/["\x27]$//') || true
...
is_workflow_disabled() {
  ...
  local state
  state=$(echo "$workflow_info" | jq -r '.state // "active"')
  echo "$state"
}
