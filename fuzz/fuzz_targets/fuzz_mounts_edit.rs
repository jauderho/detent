//! `fuzz_mounts_edit`: an arbitrary document edited with an arbitrary model
...
use detent_module_mounts::{Model, MountsModule};
...
fuzz_target!(|input: Input| {
    let Ok(mut doc) = MountsModule::parse(input.source) else {
        return;
    };
    if MountsModule::apply(&mut doc, &input.model).is_err() {
        return;
    }
